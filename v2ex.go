...
type NodeService interface {
	...
	//相当于首页右侧的 10 大每天的内容
	GetHot() (topics Topics, err error)
}

type TopicService interface {
	// id: topic ID
	GetRepliesByTopicID(id uint32) (replies Replies, err error)
	//获取节点ID下所有主题
	GetTopicsByNodeID(nodeid uint32) (topics Topics, err error)
}
...
type UserService interface {
	...
	//获取指定用户的所有主题
	GetTopicsByUsername(name string) (topics Topics, err error)
}
